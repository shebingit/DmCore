...
def head_transferred_lead(request):
    if 'emp_id' in request.session:
        ...
        leads_obj = Leads.objects.filter(lead_work_regId__in=works_obj,lead_status=1,waste_data=0,lead_transfer_status=1,lead_transfer_date=date.today()).order_by('-lead_transfer_date')
        leads_obj_count = leads_obj.count()
...
def head_lead_collected_data(request,pk,lcID):
    if 'emp_id' in request.session:
        ...
        Total_lead_count = Leads.objects.filter(lead_work_regId=works_obj,
                                        lead_category_id__id=lcID,lead_add_date=date.today()).count()
