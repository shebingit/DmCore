    ...
    if 'emp_id' in request.session:
        ...
        if request.POST:
            ...
            if d1 and d2 and (emp_id == '0') :
                ...
                leads_obj = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_add_date__gte=d1,lead_add_date__lte=d2).order_by('-lead_add_date')
                leads_obj_count = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_add_date__gte=d1,lead_add_date__lte=d2).count()

            elif d1 and d2 and (emp_id != '0'):

              
                leads_obj = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_add_date__gte=d1,lead_add_date__lte=d2,lead_collect_Emp_id__id=emp_id).order_by('-lead_add_date')
                leads_obj_count = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_add_date__gte=d1,lead_add_date__lte=d2,lead_collect_Emp_id__id=emp_id).count()
            
            elif (emp_id != '0'):

                leads_obj = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_collect_Emp_id__id=emp_id).order_by('-lead_add_date')
                leads_obj_count = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID,lead_collect_Emp_id__id=emp_id).count()
            
            else:

                

                leads_obj = Leads.objects.filter(lead_work_regId=works_obj,lead_category_id__id=lcID).order_by('-lead_add_date')
