...
def executive_lead_tracker(request):
    if 'emp_id' in request.session:
        if request.session.has_key('emp_id'):
            emp_id = request.session['emp_id']
        ...
        leads_obj = DataBank.objects.filter(lead_Id__lead_work_regId__wcompId__id=dash_details.emp_comp_id.id,
                                            lead_Id__lead_transfer_status=1,lead_Id__lead_collect_Emp_id=dash_details).order_by('-Genarated_date')
        ...
        content = {'emp_dash':emp_dash,
                    ...
                    'client_name':client_name,'category_name':category_name,'select_lead_ststus':select_lead_ststus
                    
                    }


        return render(request,'Executive_lead_Tracker.html',content)
